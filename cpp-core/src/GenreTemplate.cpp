...
std::string SectionTemplate::name() const { return sectionTypeName(type); }
...
const GenreTemplate& getGenreTemplate(GenreType type) {
  switch (type) {
    case GenreType::EDM_CHILL:
      return EDM_CHILL_TEMPLATE;
    case GenreType::EDM_DROP:
      return EDM_DROP_TEMPLATE;
    case GenreType::RETROWAVE:
      return RETROWAVE_TEMPLATE;
    case GenreType::CINEMATIC:
...
GenreType selectGenreFromImage(const ImageFeatures& features, float energy) {
  ...
  if (safeEnergy != energy) {
    std::cout << "[Genre Selection] Clamped energy from " << energy << " to "
              << safeEnergy << " (safety)" << std::endl;
  }
  ...
  if (features.brightness > 0.9f && safeEnergy > 0.7f) {
    std::cout << "[Genre Selection] Very bright + high energy → RETROWAVE "
                 "(avoiding EDM_DROP)"
              << std::endl;
  ...
  if (features.brightness > 0.6f && features.saturation > 0.4f &&
      features.saturation < 0.7f) {
