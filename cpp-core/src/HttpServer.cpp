...
void runHttpServer(int port, const std::string& defaultMode,
  ...
  svr.Post("/generate", [defaultMode, outputDir](const httplib::Request& req,
    ...
    try {
      ...
      if (mode == "heuristic") {
        ...
        res.status = 400;
        res.set_content("Unknown mode (expected 'heuristic' or 'model')",
                        "text/plain");
        return;
      }
      ...
      // Build JSON response with 7-dim parameters + Phase 7 style params
      json resp;
      resp["audio_path"] = outPath.string();
      resp["params"] = {{"tempoBpm", params.tempoBpm},
      ...
      resp["style"] = {{"ambienceType", style.ambienceType},
                       {"instrumentPreset", style.instrumentPreset},
                       {"moodScore", style.moodScore}};
      ...
      res.status = 500;
      res.set_content(std::string("Internal server error: ") + ex.what(),
                      "text/plain");
    }
  });

  std::cout << "[HTTP] Server starting on port " << port
            << " (defaultMode=" << defaultMode << ", outputDir=" << outputDir
            << ")" << std::endl;

  if (!svr.listen("0.0.0.0", port)) {
    throw std::runtime_error("Failed to start HTTP server on port " +
                             std::to_string(port));
  }
}
