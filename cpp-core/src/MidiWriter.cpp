...
void MidiWriter::setTempo(float bpm) {
    tempoBpm_ = bpm;
}
...
void MidiWriter::addNoteOn(int track, int tick, int channel, int note, int velocity) {
...
std::vector<uint8_t> MidiWriter::buildTrackData(int trackIndex) {
    ...
    if (trackIndex == 0) {
        ...
        uint32_t microsecondsPerQuarter = static_cast<uint32_t>(60000000.0f / tempoBpm_);
