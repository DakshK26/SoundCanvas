...
static int getProgramNumber(InstrumentPreset preset, int scaleType) {
    switch (preset) {
        ...
        case InstrumentPreset::KEYS:
            ...
            return (scaleType == 0 || scaleType == 3) ? 0 : 4; // Acoustic/Electric Piano
...
SongSpec makeSongSpec(const ImageFeatures& f, const MusicParameters& m) {
    ...
    if (m.energy < 0.3f || m.patternType == 0) {
        spec.groove = GrooveType::CHILL; // Sparse, ambient
    } else if (m.energy > 0.7f && spec.tempoBpm > 90) {
    ...
    bool includeDrums = (m.energy > 0.25f) && (spec.groove != GrooveType::CHILL || m.energy > 0.5f);
    if (includeDrums) {
    ...
    if (m.scaleType == 0 || m.scaleType == 3) {
        ...
        chordPreset = (m.brightness > 0.5f) ? InstrumentPreset::KEYS : InstrumentPreset::PLUCK;
    } else {
        // Minor/Dorian: warmer sounds
        chordPreset = (m.brightness > 0.5f) ? InstrumentPreset::BELL : InstrumentPreset::KEYS;
    }
    ...
    if (spec.moodScore > 0.4f) {
        ...
        spec.tracks.push_back({
            "melody",
            melodyPreset,
            getProgramNumber(melodyPreset, m.scaleType),
...
const char* instrumentPresetName(InstrumentPreset preset) {
    switch (preset) {
        ...
        case InstrumentPreset::KEYS:      return "Keys";
...
const char* grooveTypeName(GrooveType type) {
    switch (type) {
        case GrooveType::STRAIGHT: return "Straight";
        case GrooveType::CHILL:    return "Chill";
        case GrooveType::DRIVING:  return "Driving";
        default:                    return "Unknown";
    }
}
