...
int main(int argc, char** argv) {
    ...
    if (argc >= 4 && std::string(argv[1]) == "--compose-only") {
        ...
        try {
            ...
            std::cout << "  Scale: " << (songSpec.scaleType == 0 ? "Major" : 
                                        songSpec.scaleType == 1 ? "Minor" :
                                        songSpec.scaleType == 2 ? "Dorian" : "Lydian") << "\n";
            ...
            for (const auto& track : songSpec.tracks) {
                std::cout << "  " << track.role << ": " 
                          << instrumentPresetName(track.preset)
                          << " (program " << track.midiProgram << ")"
                          << " vol=" << track.volume
            ...
            std::cout << "  fluidsynth -a alsa soundfont.sf2 " << outputMidi << std::endl;
    ...
    if (argc < 3 || argc > 4) {
        std::cerr << "Usage:\n"
                  ...
                  << "  soundcanvas_core --compose-only <input_image> <output_midi>           → MIDI composition only\n"
