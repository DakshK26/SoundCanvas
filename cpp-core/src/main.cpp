...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const char* ambienceNames[] = {"None", "Ocean", "Rain", "Forest", "City"};
    const char* instrumentNames[] = {"Soft Pad", "Soft Keys", "Pluck", "Bell"};

    const char* ambienceName =
        (style.ambienceType >= 0 && style.ambienceType <= 4)
            ? ambienceNames[style.ambienceType]
            : "Unknown";
    const char* instrumentName =
        (style.instrumentPreset >= 0 && style.instrumentPreset <= 3)
            ? instrumentNames[style.instrumentPreset]
            : "Unknown";

    std::cout << "Style parameters (Phase 7):\n"
              << "  ambienceType     = " << style.ambienceType << " ("
              << ambienceName << ")\n"
              << "  instrumentPreset = " << style.instrumentPreset << " ("
              << instrumentName << ")\n"
              << "  moodScore        = " << style.moodScore << std::endl;
